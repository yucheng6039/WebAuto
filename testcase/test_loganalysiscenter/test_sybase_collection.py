import allure
import pytest
from pages.loganalysiscenter_page.sybase_collection import LogHome
...
def sign_in(login_as):
    ...
    yield page
@allure.feature("sybase数据库采集")
class TestLogHome(object):
    @allure.title("创建sybase采集任务")
    def test_home(self,sign_in):
        ...
        page.click_element("//span[text()='Sybase']")
        page.key1.send_keys("192.168.32.37")
        page.key2.send_keys("5000")
        page.key3.send_keys("master")
        page.key4.send_keys("sa")
        page.key5.send_keys("password")
        ...
        sleep(1)
        page.key7.send_keys('select * from hai')
        page.click_element("//button[text()='数据预览']")
        sleep(4)
        page.click_element("//span[text()='增量同步']")
