import allure
import pytest
from pages.loganalysiscenter_page.windows_file_collection import LogHome
...
def sign_in(login_as):
    ...
    yield page
@allure.feature("windows文件采集")
class TestLogHome(object):
    @allure.title("创建windows文件采集任务")

    @allure.step("这是一个windows文件采集步骤")
